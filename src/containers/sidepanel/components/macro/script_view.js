...
const UIV_METHODS = [
  ...
  { text: 'tabs.select(', displayText: 'tabs.select(2) - switch to tab #2 (ABSOLUTE, 1-based, left to right); returns {index, title, url} so the script can verify where it landed' },
  { text: 'tabs.list(', displayText: 'tabs.list() -> all tabs of the window as [{index, title, url, active}, ...]' },
  { text: 'tabs.open(', displayText: 'tabs.open(url) - NEW tab on url, waits for load (uiv.open navigates the CURRENT tab instead); returns {index, title, url}' },
  ...
  { text: 'getVar(', displayText: "getVar('!URL' | 'name'[, default]) - read a Ui.Vision variable, special ones included (!CURRENT_TAB_NUMBER, !LASTCOMMANDOK, !COL1, !IMAGEX); throws on an unknown name or one that is not set yet" },
  { text: 'setVar(', displayText: "setVar('name' | '!TIMEOUT_PAGELOAD', value) - write a variable; readonly system vars (!URL, !CURRENT_TAB_NUMBER, ...) are rejected" },
