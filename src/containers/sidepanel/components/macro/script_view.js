...
const UIV_METHODS = [
  ...
  { text: 'getVar(', displayText: "getVar('name' | '!LASTCOMMANDOK'[, default]) - read a Ui.Vision variable, special ones included (!COL1, !IMAGEX); throws on an unknown name or one that is not set yet. !URL and !CURRENT_TAB_NUMBER are table-macros-only and throw here - the page URL is eval('return location.href'), the tab position is tabs.list() (current: true entry)" },
