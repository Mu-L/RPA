...
import { isFreeTierConsentPending, mapUIVisionFreeTierError } from '../uivision_free_tier'
...
const getSystemPrompt = (xmoduleStatus: string): string => {
...
export class MacroAgentService {
  ...
  private runAnthropic = async (apiKey: string, model: string, userText: string): Promise<void> => {
    ...
    for (let loop = 0; loop < this.getMaxLoops(); loop++) {
      ...
      const response = await anthropic.messages.create({
        model,
        max_tokens: 4096,
        system: [{ type: 'text', text: getSystemPrompt(this.xmoduleStatus), cache_control: { type: 'ephemeral' } } as any],
        tools: tools as any,
  ...
  private runOpenAICompatible = async (
    ...
    if (this.messages.length === 0) {
      ...
      this.messages.push({ role: 'system', content: getSystemPrompt(this.xmoduleStatus) })
    }
    this.messages.push({ role: 'user', content: [{ type: 'text', text: userText }] })
    ...
    for (let loop = 0; loop < this.getMaxLoops(); loop++) {
      ...
      this.params.logMessage(providerLabel === 'Ui.Vision AI' ? `Calling API (${providerLabel})` : `Calling API (${providerLabel} ${model})`, 'status')
