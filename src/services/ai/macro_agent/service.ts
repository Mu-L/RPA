...
const getSystemPrompt = (xmoduleStatus: string, localOcrAvailable: boolean): string => {
  ...
  const cloudHint = cfg.ocrSpaceApiKey
    ? 'an OCR.Space API key IS configured — when local OCR reads badly, use {engine: 2} (finders/clicking) or {engine: 3} (best pure-text reads; x/y less accurate — coordinate fallback only when 2 fails); both auto-detect the text language'
  ...
  const ocrNote = [1, 2, 3].includes(ocrEngine)
    ? `default engine is OCR.Space cloud (engine ${ocrEngine})${ocrEngine === 1 ? ' — engine 1 reads worst of the cloud engines: write {engine: 2} (clicking) or {engine: 3} (pure reads) into OCR steps instead; both auto-detect the text language' : ''}`
    : `default engine ${ocrEngine} (local); XModule Local OCR (engine 99): ${localOcrAvailable ? 'AVAILABLE — when an OCR step needs a better reader than the Javascript OCR, write {engine: 99} into that step (much better on native UI and screenshots, and the go-to reader for desktop scope)' : 'not installed'}; ${cloudHint}`
...
export class MacroAgentService {
  ...
  run = async (prompt: string): Promise<void> => {
    ...
    // tells the agent to WRITE {engine: 99} into OCR steps when available —
